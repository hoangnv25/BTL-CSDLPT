...
export default function InventoryWareHourse({ warehouse_id = 1 }) {
  ...
  // Fetch tồn kho dựa trên warehouse_id
  const fetchInventories = async () => {
    setLoading(true);
    setError('');
    try {
      const endpoint = `${API_BASE_URL}/inventory/by_warehouse?warehouse_id=${warehouse_id}`;
  ...
  // Gọi fetchInventories mỗi khi warehouse_id thay đổi
  useEffect(() => {
    fetchInventories();
  }, [warehouse_id]);
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.header}>
        <h1 className={styles.title}>Quản Lý Tồn Kho - Kho #{warehouse_id}</h1>
      </div>
